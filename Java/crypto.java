...
{	
	...
	throws IOException
	{
		...
		/** 
		 * This method stores text into a file of the user's choosing
		 ...
		 * Duplicates within the key are removed
		 */
		public static void keySub()
		{
			...
			for(int i=0;i<yo.length;i++){tempSubKey[i]=yo[i].toString();}
			int counterChars1 = 0;
			charKeySub=new char[yo.length];
			for (int i = 0; i < tempSubKey.length; i++)
			{ 
				int counterLetters1 = 0;  
		        for (int j = 0; j < tempSubKey[i].length(); j++){charKeySub[counterChars1] = tempSubKey[i].charAt(counterLetters1);counterLetters1++;counterChars1++;}
			}
			removeDuplicate();
		}
		/** 
		 * This method converts an array holding the key and a full listing of the
		 * alphabet to a Linked Hash Set which retains the order and removes duplicates
		 * Then the LHS is converted back into a char[]
		 */
		public static void removeDuplicate()
		{
			...
			for(int i=0;i<charKeySub.length;i++){alphabet2[i] = charKeySub[i];}
			for(int i=0;i<alphabet.length;i++)
			{
				for(int j=0;j<charKeySub.length;j++){alphabet2[charKeySub.length+i]=alphabet[i];}
			}
			for(int i=0;i<charKeySub.length+alphabet.length;i++){alpha.add(alphabet2[i]);}
			Object[] objects=alpha.toArray();
			String tempAlpha[]  = new String[objects.length];
			for(int i=0;i<tempAlpha.length;i++){tempAlpha[i]=objects[i].toString();}
			int counterChars = 0;
			Sub=new char[objects.length];
			for (int i = 0; i < tempAlpha.length; i++)
			{ 
				int counterLetters = 0;  
		        for (int j = 0; j < tempAlpha[i].length(); j++){Sub[counterChars] = tempAlpha[i].charAt(counterLetters);counterLetters++;counterChars++;}
			}
		}
		/**
		 * This method uses a two dimensional array to store and manipulate the alphabet + the keyword
		...
		public static void transposition()
		{
			...
			for(int i=0;i<rows;i++)
			{
				...
				if(extra>0 && i==rows-1)
				{i++;for(int k=0;k<extra;k++){subcipher[i][k]=Sub[n];n++;}}
			}
			n=0;
			...
			for(int i=0;i<alphabet.length;i++){Sub[i] = Character.toUpperCase(Sub[i]);}
			for(int i=0;i<alphabet.length;i++){alphabet[i] = Character.toUpperCase(alphabet[i]);}
		}
		/** 
		...
		public static void keyVigenere()
		{
			...
			for(int i=0;i<toText.length();i++){keyV[i] = Character.toUpperCase(keyV[i]);}
		}
		/** 
		...
		/**
		 * This method encrypts the message using the Shift algorithm 
		 */
		...
		/**
		 * This method decrypts the message using the Shift algorithm 
		 */
		...
		/**
		 * This method encrypts the message using the Substitution algorithm 
		 */
		...
		/**
		 * This method decrypts the message using the Substitution algorithm 
		 */
		...
		/**
		 * This method encrypts the message using the Affine algorithm 
		 */
		...
		/**
		 * This method decrypts the message using the Affine algorithm 
		 */
		...
		/**
		 * This method encrypts the message using the Vigenere algorithm 
		 */
		...
		/**
		 * This method decrypts the message using the Vigenere algorithm 
		 */
